...
class User {
  ...
  public resolveImageUrl() {
    if (
      ...
      this.information?.imageurl != null
    ) {
  ...
  public resolveBackImageUrl() {
    if (
      this.information?.banelImage != "" &&
      this.information?.banelImage != null
    ) {
  ...
  public addBox(newBox: Box) {
    if (
      this.boxs.filter((item) => item.getTypeName() == newBox.getTypeName())
        .length > 0
    ) {
      this.boxs.splice(
        this.boxs.findIndex(
          (item) => item.getTypeName() == newBox.getTypeName()
        ),
        1
      );
    }
