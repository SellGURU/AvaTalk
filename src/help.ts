...
const resolveMenuFromRoute = () => {
  ...
  switch (window.location.hash.replace("#/", "").replace("?splash=false", "").split("/")[0]) {
    ...
    case "?splash=true&signin_success=true":
      return 'profile';
    case "?splash=false&signin_success=true":
      return 'profile';            
    case "?splash=true&signup_success=true":
      return 'profile';       
    case "?splash=false&signup_success=true":
      return 'profile';               
    case "analytics":
      return 'status';      
    default:
...
const boxProvider = (box: any) => {
  switch (box.type_name) {
    ...
    case "GoogleMapBox": {
      ...
      return Object.assign(new GoogleMapBox("simple",{lan:0,lat:0},'',false), {...box,type_name:'AddressBox'});
    }    
    case "AddressBox": {
