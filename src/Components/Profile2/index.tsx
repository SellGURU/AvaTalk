...
const Profile2: React.FC<ProfileProps> = ({ theme }) => {
  ...
  const [mode, setMode] = useState<"profile" | "review" | "share">(
    resolveMode()
  );
  ...
  const [suggestionList, setSuggestionList] = useState(
    authContext.currentUser.sugesstions
  );
  useEffect(() => {
    if (isTalking) {
      if (videoRef2.current) {
  ...
  useConstructor(() => {
    ...
    if (id) {
      ...
      const resolveSocial: Array<Box> = [];
      Share.getShareData(
        "/presentation_info/user=" + searchParams.get("user"),
        (data) => {
          if (data.error) {
          ...
          if (localStorage.getItem("showTotorial" + searchParams.get("user"))) {
            ...
            localStorage.setItem(
              "showTotorial" + searchParams.get("user"),
              "true"
            );
          }
          shareUser.setTypeOfAccount(
  ...
  return (
    <>
        ...
        <div className="w-full h-svh relative flex justify-start text-gray-700 text-center flex-col">
          ...
          >
            ...
            {mode == "profile" ? (
              <>
                <div
                  ref={ButtonnotificationRefrence}
                  className="absolute grid gap-2 top-4 right-6 z-20"
                >
                  <Button
                    onClick={() => {
                      setShowNotification(!showNotification);
                ...
                <div className="absolute grid gap-2 top-4 left-6 z-20">
                  ...
                  >
                    <div className="flex w-full items-center pl-2 justify-center ">
                      <div
                        className={`${theme}-Profile-EditProfileBtnVector2 ${theme}-Footer-Vectors m-0`}
                      ></div>
                    </div>
                ...
                <div className="absolute top-28 right-6 z-20">
                  <Button
                    onClick={() => {
                      setIsTalking(false);
                      window.open(
                        "https://ar-card-develop.vercel.app/#detect7/?user=" +
                          shareUser.information?.unique_id +
                          "&view=" +
                          mode
                      );
                    }}
                    ...
                    <div className="absolute top-4 right-6 z-20">
                      <Button
                        onClick={() => {
                          ...
                          if (
                            shareUser.type_of_account.getType() == "Free" &&
                            mode == "share"
                          ) {
                            setISMuted(true);
                            // console.log(chats)
            ...
            <div
              className={`w-full mt-[-320px]  invisible py-4 px-4 pb-0 -mb-2  ${
                scrolled
                  ? "profileAimation3"
                  : isFirstScrol
                  ? "profileAimation3-backward"
                  : ""
              } `}
            >
              <div
                className={`w-full relative overflow-hidden bg-white ${
                  mode != "profile"
                    ? "h-[160px] -mt-[16px]"
                    : " h-[130px] -mt-[20px]"
                }  rounded-[16px] flex items-center justify-start boxShadow-Gray`}
              >
                <img
            ...
            <div
              className={`w-full ${
                mode != "profile" ? "mt-[140px]" : "mt-[170px]"
              }  overflow-hidden relative  h-[231px]  ${
                scrolled
                  ? "profileAimation2-mobile md:profileAimation2"
                  : "profileAimation2-backward-mobile md:profileAimation2-backward"
              }   md:h-[231px] bg-white rounded-3xl rounded-t-none  pb-4  flex flex-col `}
              style={{
                boxShadow: !window.location.href.includes("/edit")
                  ? "0px 4px 12px 0px #6F8CB069"
                  : "unset",
              }}
            >
              <img
                ...
                <div className="relative">
                  <div className="w-[146px] border-4 relative border-primary-color flex justify-center items-center h-[146px] rounded-full overflow-hidden">
                    ...
                    >
                      <source
                        id="videoPlayer3"
                        src={shareUser.information?.silent_video_avatar}
                        type="video/mp4"
                      ></source>
                    </video>
                    ...
                    {prisentMode == "video" ? (
                      <>
                        <video
                          ...
                          preload="auto"
                          muted={prisentMode == "video" ? false : true}
                          autoPlay={prisentMode == "video" ? false : true}
                          loop={prisentMode == "video" ? false : true}
                        >
                          <source
                            id="videoPlayer2"
                            src={
                              prisentMode == "video"
                                ? VideoUrl
                                : shareUser.information?.talk_video_avater
                            }
                            type="video/mp4"
                          ></source>
                        </video>
                      ...
                      <>
                        ...
                        <video
                          ...
                          playsInline
                          width={"100%"}
                          className={`pk_video h-full w-full object-cover absolute ${
                            isTalking && startVideoTalk
                              ? "visible"
                              : "invisible"
                          } ${
                            window.innerWidth > 600 ? "mt-[0px]" : "mt-[0px]"
                ...
                {shareUser.boxs && shareUser.boxs.length > 0 ? (
                  ...
                  <>
                    ...
                    <p className="text-sm	text-[#8290a3]">
                      You haven't added any info yet.
                    </p>
                  </>
                )}
                {mode != "profile" &&
                shareUser.boxs.filter((el) => el.isShareAble() == true).length >
                  0 ? (
                  <div className="w-full  pb-4  flex justify-start items-center">
                    ...
                    <div className=" leading-[15px] flex-grow text-justify  font-normal text-[10px]  sm:text-[13px]  ml-1 ">
                      ...
                      <span
                        ...
                        className="text-primary-color text-justify font-medium cursor-pointer"
                      >
                        Avatalk
                      </span>
                      ? Create your AI Avatar in less than 3 minutes!
                    </div>
                  </div>
              ...
              {mode != "profile" && !isEditPage() ? (
                <>
                  <div className=" absolute w-full z-20 bottom-0">
                    {mode == "review" &&
                    shareUser.boxs.filter((el) => el.isShareAble() == true)
                      .length == 0 ? (
                      <div className="w-full bg-white pb-4 text-justify flex justify-start items-center">
                        ...
                        <div className=" ml-1 font-normal text-[10px] sm:text-[13px] leading-[15px]">
                          ...
                          <span
                            ...
                            className="text-primary-color font-medium cursor-pointer"
                          >
                            Avatalk
                          </span>
                          ? Create your AI Avatar in less than 3 minutes!
            ...
            <>
              {!isLoading && (
                <Presentition2
                  ...
                  mode={mode}
                  setPrisentMode={setPrisentMode}
                  setVideoUrl={setVideoUrl}
                  setShowMuiteController={setShowMuiteController}
                  isSilent={isMuted}
                  chats={chats}
                  setChats={setChats}
                  shareUser={shareUser}
      ...
      <AudioProvider
        setStartVideoTalking={setStartVideoTalk}
        autoPlay={isTalking}
        onEnd={() => {
          setAudioUrl("");
