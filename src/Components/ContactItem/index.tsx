        ...
        <div className={`${theme}-ContactItem-card `}>
          <div className={`${theme}-ContactItem-innerCard `}>
            ...
            <div className={`${theme}-ContactItem-iconContainer  `}>
              {data.tags.length == 1 && <p className={`${theme}-ContactItem-exhibition `} style={{backgroundColor:data.tags[0].color}}>{data.tags[0].name}</p>}
              {data.tags.length > 1 ?
                <div  onClick={() => {setShowMoreTags(!showMoreTages)}} className="flex items-center cursor-pointer justify-start">
                  {data.tags.map((item,index) => {
                    return (
                      <>
                        {index <3 ? 
                          <div id="tags" className={`w-6 h-6 rounded-full -ml-3 border-2 border-white`} style={{backgroundColor:item.color}}></div>
                        :undefined}
               ...
               :undefined}
               {showMoreTages && <div id="tags"  className="bg-[#F3F4F6] border-2 z-20 right-2 border-white rounded-[15px] absolute py-2 top-14 overflow-y-scroll max-h-[110px] w-[123px]">
                  {data.tags.map((item,index) => {
                    return (
                      <>
