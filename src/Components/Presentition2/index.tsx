...
const TextWithNewlinesAndLinks: React.FC<TextWithNewlinesAndLinksProps> = ({
  text,
}) => {
...
const Presentition2: React.FC<PresentationProps> = ({
  theme,
  chats,
  mode,
  suggestions,
  ...
  const [languagesList, setLanguagesList] = useState([
    {
      lan: "English",
      code: "en-US",
      icon: "./icons/country/Great Britain.png",
    },
    ...
    { lan: "French", code: "fr", icon: "./icons/country/France.png" },
    {
      lan: "Arabic",
      code: "ar-AE",
      icon: "./icons/country/United Arab Emirates.png",
    },
  ...
  const handleLanguageSelect = (selectedCode: any) => {
    setLanguagesList((prevList: any) => {
      ...
      const fixedLanguages = ["en-US", "de", "fr"].filter(
        (el) => el != selectedCode
      );
      const fixedLangs = prevList.filter((lang: any) =>
        fixedLanguages.includes(lang.code)
      );

      // Find the selected language
      const selectedLang = prevList.find(
        (lang: any) => lang.code === selectedCode
      );
      ...
      const otherLangs = prevList.filter(
        (lang: any) =>
          lang.code !== selectedCode && !fixedLanguages.includes(lang.code)
      );
  ...
  const [selectedLang, setSelectedLang] = useState(
    localStorage.getItem("chatLanguage")
      ? JSON.parse(localStorage.getItem("chatLanguage") as string)
      : languagesList[0]
  );
  useEffect(() => {
  ...
  useConstructor(() => {
    ...
    if (
      mode == "review" &&
      context.prerecorded_voice != null &&
      chats.length == 0
    ) {
      // alert("this hear")
      ...
      if (
        context.currentUser.type_of_account.getType() == "Pro" &&
        context.currentUser.type_of_account.getDayremindToExpired() <= 7
      ) {
        // setTimeout(() => {
        //      setFirstComeSuggestion(true)
        // }, 100);
  ...
  const handleSendVector = (value: string) => {
    ...
    sendToApi(
      ...
      (res) => {
        ...
        if (
          !isSilent &&
          !(res.answer.audio_file == null && res.answer.video_file == null)
        ) {
          setIsTalking(true);
        }
      ...
      selectedLang.lan,
      BLokedIdList,
      shareUser?.information?.userId as string,
      isSilent
    );
  };
  ...
  return (
    <>
      <div
        className={`${theme}-Presentation-MoreInfoSection px-4 pt-2 pb-36 ${theme}-Presentation-fadeIn`}
      >
        {showSuggestions && suggestions.length > 0 && chats.length == 0 ? (
          <>
            <Suggestions
              title="Ask me more information"
              theme="Carbon"
              onVSelectItem={(text: string | null) => {
                handleSendVector(text as string);
              }}
            ...
            (context.currentUser.type_of_account.getType() == "Free" ||
              ...
              <div className="flex w-full justify-start">
                <div className={`${theme}-Presentation-chatItem`}>
                  ...
                  {context.currentUser.type_of_account.getType() == "Trial" ? (
                    <div>
                      <TextWithNewlinesAndLinks
                        text={`"Hi, I am your Avatar. Your trial gives you full access to Avatalk's networking power, for a limited time. Keep your avatar"`}
                      />
                      <div
                        onClick={() => {}}
                        className="flex cursor-pointer  mt-[16px] gap-2 items-center justify-end"
                      >
                    ...
                    <div>
                      <TextWithNewlinesAndLinks
                        text={`Hi, I am your Avatar. Unlock voice chat for your Avatalk with a Pro Subscription! Every new user can have a conversation up to 2 messages. Get Pro to let users interact with Avatalk as much as they want! Don't worry, Text chat is always unlimited!`}
                      />
                  ...
                  {item.from == "user" ? (
                    <>
                      ...
                      <div className="flex w-full justify-end">
                        <div className={`${theme}-Presentation-AnswerTitle`}>
                          {item.text}
                        </div>
                      </div>
                    </>
              ...
              <>
                <div className="  w-full px-4 flex justify-between items-center rounded-full h-10 borderBox-Gray2 bg-slate-100 ">
                  ...
                  <div
                    ...
                    onClick={() =>
                      handleStop(chats[chats.length - 1].message_key)
                    }
                  >
                    stop
                  </div>
        ...
        <div className="absolute z-40 bottom-20 right-2">
          <MultiLan
            handleLanChange={handleLanguageSelect}
            setShowMore={(act) => {
        ...
        {mode != "share" ||
          (isVoceEnded && (
            <div className=" absolute bottom-10 bg-white z-50 py-4 mt-24  mb-[24px]">
              <AccessNotifManager
                modeLimited={resolveModeNotif() as string}
                page="chatEndUser"
              ></AccessNotifManager>
