...
const AccessNotifManager: React.FC<AccessNotifManager> = ({
  page,
  isLimited,
  modeLimited,
}) => {
  ...
  useEffect(() => {
    ...
    if (
      authContext.currentUser.type_of_account.getType() == "Free" &&
      !isSkipped
    ) {
      setTimeout(() => {
        setShowNotif(true);
    ...
    if (
      authContext.currentUser.type_of_account.getType() == "Pro" &&
      !isSkipped
    ) {
  ...
  const resolveText = () => {
    ...
    if (authContext.currentUser.type_of_account.getType() == "Pro") {
      if (
        page == "chatEndUser" &&
        authContext.currentUser.type_of_account.getDayremindToExpired() <= 7
      ) {
  ...
  return (
    <>
      {showNotif ? (
        <div
          className={`w-full py-4 px-6 rounded-[27px] bg-white borderBox-primary3 ${
            authContext.currentUser.type_of_account.getType() == "Trial" &&
            page == "chatEndUser"
              ? "border-none rounded-tl-none"
              : "borderBox-primary3"
          } `}
        >
          <div className="text-[14px] text-justify text-[#374151]">
            {resolveText()}
          </div>
          {authContext.currentUser.type_of_account.getType() == "Trial" &&
          page != "chatEndUser" ? (
            <>
              <div
                className="w-full h-2 relative rounded-[12px] bg-gray-200 border-[#FFFFFF80] border mt-4"
                style={{
                  boxShadow:
                    "inner 4px 4px 14px 2px #E2E8F0,inner -4px -4px 9px 2px #FFFFFF99",
                }}
              >
                <div
                  className={`absolute left-0 h-[7px] top-0 rounded-[12px] bg-[#FBBF24] `}
                  style={{
                    width:
                      authContext.currentUser.type_of_account.getPercentDayUsed() +
                      "%",
                  }}
                ></div>
              </div>
          ...
          <div className="flex w-full font-medium mt-2 gap-6 justify-end items-center">
            {
              modeLimited == "endUser" && page == "chatEndUser" ? (
                ...
                <div
                  ...
                  className="text-[#FBBF24] cursor-pointer font-semibold text-[14px]"
                >
                  Create Your Avatalk
                </div>
                <div
                    ...
                    className="text-[#94A3B8] cursor-pointer text-[14px]"
                  >
                    Skip Now
                  </div>
                </>
                ...
                <>
                  <div
                    ...
                    className="text-[#FBBF24] font-semibold cursor-pointer text-[14px]"
                  >
                    {authContext.currentUser.type_of_account.getType() == "Pro"
                      ? "Renew Your Plan"
                      : "Upgrade to Pro"}
