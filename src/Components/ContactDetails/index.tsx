    ...
    if (contactId) {
      Auth.getContactDetails(contactId, (contactDetails) => {
        const newContact: Contact = {
          ...
          photo: contactDetails.profile_pic
            ? contactDetails.profile_pic
            : "https://ui-avatars.com/api/?name=" + contactDetails.full_name,
      ...
      <div className={`${theme}-ContactDetails-container2`}>
        <div className={`${theme}-ContactDetails-container3`}>
          <div className={`${theme}-Profile-ProfilePictureSection`}>
            <img
              src={`https://ui-avatars.com/api/?name=` + contact?.fullName}
              alt={contact?.fullName}
              className={`${theme}-Profile-ProfilePicture `}
            />
          </div>
          <div className={`${theme}-ContactDetails-importIconContainer`}>
            <div
              ...
              className={`${theme}-ContactDetails-importIcon `}
            ></div>
            <div
              onClick={() => setShowEditContactModal(true)}
              className={`${theme}-ContactDetails-editIcon`}
            ></div>
            <div
              onClick={() => setShowDeleteContactModal(true)}
              className={`${theme}-ContactDetails-recycleIcon`}
            ></div>
          </div>
        </div>
        <p
          className={`${theme}-ContactDetails-nameItem mt-2 break-words text-center w-[80%]`}
        >
          {contact?.fullName}
        </p>
        <p className={`${theme}-ContactDetails-jobItem break-words w-[80%]`}>
          {contact?.job}
        </p>
        <div className={`${theme}-ContactDetails-showExibitionconContainer `}>
          ...
          {!showAddTagModal && (
            <>
              {contact?.tags.map((item, index) => {
                return (
                  <>
                    {index < 2 ? (
                      <div
                        className={`${theme}-ContactDetails-exibitionconContainer px-2`}
                        style={{ backgroundColor: item.color }}
                      >
                        <p
                          className={`${theme}-ContactDetails-exibition`}
                          style={{
                            color: getTextColorFromBackground(item.color),
                          }}
                        >
                          {item.name}
                        </p>
                        <div
                          ...
                          className={` ${theme}-ContactDetails-crossIcon  `}
                          style={{ width: "24px" }}
                        ></div>
                      </div>
                    ) : undefined}
                    {index == 2 && (
                      <div
                        onClick={() => setShowMoreTags(!showMoreTages)}
                        className="w-8 h-8 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center cursor-pointer"
                      >
                        <div id="tags" className="text-gray-700 -mt-2">
                          ...
                        </div>
                      </div>
                    )}
              ...
              {showMoreTages && (
                <div
                  id="tags"
                  ref={showMoreRefrence}
                  className="bg-[#F3F4F6] border-2 z-20 right-0 border-white rounded-[15px] absolute py-2 top-10 overflow-y-scroll hiddenScrollBar max-h-[110px] w-[123px]"
                >
                  {contact?.tags.map((item, index) => {
                    return (
                      <>
                        {index >= 2 ? (
                          <>
                            <div className="w-full items-center justify-center flex">
                              <div
                                className={`${theme}-ContactItem-exhibition `}
                                style={{ backgroundColor: item.color }}
                              >
                                {item.name}
                              </div>
                            ...
                            {index < contact.tags.length - 1 && (
                              <div className="w-full my-1 border-white  border-t" />
                            )}
                          </>
                        ) : undefined}
                      </>
          ...
          {!showAddTagModal && (
            <div
              ...
              className="text-[#06B6D4] hidden text-[14px] font-medium cursor-pointer"
            >
              Add Tag
            </div>
          )}
        ...
        <div className={`${theme}-ContactDetails-container4 min-w-64`}>
          ...
          {contact?.address ? (
            <div className={`${theme}-ContactDetails-container5`}>
              ...
              >
                <div
                  className={`${theme}-ContactDetails-Vectors ${theme}-ContactDetails-locationIcon ${theme}-ContactDetails-ActiveVectors`}
                ></div>
              </div>
              <p
                className={`${theme}-ContactDetails-textItem max-w-[350px] `}
                style={{
            ...
            <>
              {contact?.company ? (
                <div className={`${theme}-ContactDetails-container5`}>
                  ...
                  >
                    <div
                      className={`${theme}-ContactDetails-Vectors ${theme}-ContactDetails-buildingIcon ${theme}-ContactDetails-ActiveVectors`}
                    ></div>
                  </div>
                  <p className={`${theme}-ContactDetails-textItem break-words`}>
                    {contact?.company}
                  </p>
                </div>
              ) : undefined}
              {contact?.note ? (
                <div className={`${theme}-ContactDetails-container5`}>
                  ...
                  >
                    <div
                      className={`${theme}-ContactDetails-Vectors ${theme}-ContactDetails-meetingIcon ${theme}-ContactDetails-ActiveVectors`}
                    ></div>
                  </div>
                  <p className={`${theme}-ContactDetails-textItem break-words`}>
                    {contact?.note}
              ...
              {contact?.addDate ? (
                <div className={`${theme}-ContactDetails-container5`}>
                  ...
                  >
                    <div
                      className={`${theme}-ContactDetails-Vectors ${theme}-ContactDetails-calendarIcon ${theme}-ContactDetails-ActiveVectors`}
                    ></div>
                  </div>
                  <p className={`${theme}-ContactDetails-textItem`}>
                    {" "}
                    Added on {TimeManegar.formatDate(contact?.addDate)}
                  </p>
                </div>
              ) : undefined}
            </>
