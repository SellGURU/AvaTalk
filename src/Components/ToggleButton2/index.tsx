...
const ToggleButton2: React.FC<ToggleProps> = ({ theme,value, leftText, rightText, onButtonClick }) => {
  ...
  return (
    <div className={`${theme}-ToggleButton-container px-2 `}>
      <div className={`${theme}-ToggleButton-notActiveButton border border-solid border-transparent px-5   ${activeButton === leftText ? activeStyle : ""} `}>
        <Button onClick={() => handleButtonClick(leftText)} theme="Carbon-Toggle">
          <img src="./Carbon/userVector.svg" className=" w-5 h-5 me-[6px]"/>
          {leftText}
        </Button>
      </div>
      <div className={`${theme}-ToggleButton-notActiveButton border border-solid border-transparent px-5  ${activeButton === rightText ? activeStyle : ""} `}>
