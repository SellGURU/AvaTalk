...
${
  contact.socialProfiles.linkedin
    ? `URL:${contact.socialProfiles.linkedin}`
    : ""
}
${
  contact.socialProfiles.instagram
    ? `URL:${contact.socialProfiles.instagram}`
    : ""
}
END:VCARD
  `.trim();
...
const ShowUser: React.FC<ShowUserProps> = ({
  ...
  const mettingBox: any = user?.boxs.filter(
    (el) => el.getTypeName() == "MeetingBox"
  )[0];
  const socialsBox: any = user?.boxs.filter(
    (el) => el.getTypeName() == "SocialBox"
  )[0];
  return (
    <>
      <div
        ref={refEl}
        className="rounded-[27px] px-6 py-6 max-w-[32rem] h-auto max-h-[678px] pb-10 rounded-b-none slideupModal  bg-white w-full"
      >
        {step == 0 ? (
          <>
            <div className="flex w-full pt-0 justify-between items-start">
              ...
              <div>
                <div className="text-text-primary text-[18px] font-medium break-words contactNameShadow">
                ...
                <div
                  className={`text-text-primary ${
                    user.information?.company && user.information?.job
                      ? "visible"
                      : "invisible"
                  } mt-1 text-center text-xs flex flex-wrap justify-center items-center `}
                >
                  <>{user.information?.job}</>
                  <div className="flex items-center justify-center flex-wrap ml-1">
                    {" @ "}
                    <span className="ml-1">
                      <img
                        className={`w-[16px] ${
                          user.information?.logo ? "block" : "hidden"
                        } h-[16px]`}
                        src={user.information?.logo}
                        alt=""
                      />
                    </span>
            ...
            <div className="my-4 flex flex-col gap-5 font-medium text-sm">
              <a
                href={"https://portal.avatalk.me" + user?.resolveLink()}
              ...
              >
                ...
                >
                  <div
                    className={`${theme}-ContactDetails-Vectors ${theme}-ContactDetails-avatalkIcon ${theme}-ContactDetails-ActiveVectors`}
                  ></div>
                </div>
                <p
                  className={`${theme}-ContactDetails-textItem cursor-pointer break-words`}
                >
                  {"portal.avatalk.me" + user?.resolveLink()}
                </p>
              </a>

              {user?.information?.phone ? (
                <a
                  href={`tel:+${user?.information?.phone}`}
                  className={`${theme}-ContactDetails-container5`}
                >
                  <div
                    className={`${theme}-ContactDetails-VectorSection ${theme}-ContactDetails-ActiveVectorSection`}
                  ...
                  <p
                    className={`${theme}-ContactDetails-textItem cursor-pointer`}
                  >
                    {"+" + user?.information?.phone}
                  </p>
                </a>
              ...
              {user?.information?.personlEmail ? (
                <a
                  href={`mailto:${user?.information?.personlEmail}`}
                  className={`${theme}-ContactDetails-container5`}
                >
                  <div
                    className={`${theme}-ContactDetails-VectorSection ${theme}-ContactDetails-ActiveVectorSection`}
              ...
              {user?.information?.address ? (
                <a className={`${theme}-ContactDetails-container5 `}>
                  <div
                    className={`${theme}-ContactDetails-VectorSection ${theme}-ContactDetails-ActiveVectorSection`}
                  >
                    <div
              ...
              {user?.boxs.filter((el) => el.getTypeName() == "MeetingBox")
                .length > 0 && mettingBox.getUrl() != "" ? (
                <a
                ...
                >
                  ...
                  <p
                    className={`${theme}-ContactDetails-textItem cursor-pointer`}
                  >
                    {mettingBox.getUrl().substring(0, 50)}
                  </p>
                </a>
              ) : undefined}
              {user?.boxs.filter((el) => el.getTypeName() == "SocialBox")
                .length > 0 && socialsBox.getSocialMedias().length > 0 ? (
                <a className={`${theme}-ContactDetails-container5 `}>
          ...
          <>
            <div>
              <div className="flex pt-4 w-full justify-between items-center">
                ...
                <div>
                  <div className="text-text-primary text-[14px] font-semibold contactNameShadow">
                    Contact Saved!
                  </div>
                </div>
                <div className="z-30">
              ...
              <div className="text-[14px] font-medium mt-6 text-center text-text-primary">
                Want to manage your network this easily? Activate your free
                Avatalk trial and take control
              </div>
              <div className="w-full flex justify-center mt-5">
                <div className="w-[240px] h-[368px] bg-primary-color rounded-[30px]">
                  ...
                  <div>
                    ...
                    <div className="text-white text-[12px] font-medium opacity-85  text-center">
                      Job Title/ Company
                    </div>
                  </div>
                  <div className="w-full flex justify-center mt-4">
