...
const Chat: React.FC<Props> = ({ theme }) => {
  ...
  const filteredData = chats?.filter(
    (item) =>
      item.chat_list_id.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className={`${theme}-ContactsView-Container relative `}>
      ...
      <div className={`${theme}-ContactsView-buttonsContainer w-3/4 `}>
        <ToggleButton
          ...
          leftText="Visitors Chat History"
          rightText="Your Test History"
          theme="Carbon"
        />
      </div>
          ...
          {chats?.length == 0 ? (
            <div className={`${theme}-ContactsView-box w-[100%] mt-[20px]`}>
              <div
                ...
                className={` w-[100%]  ${theme}-ContactsView-innerBox`}
              >
                No Chat Yet
              </div>
            </div>
            ...
            <div>
              <ChatList
                isLimitedChat={
                  appcontext.currentUser.type_of_account.getType() == "Free"
                    ? true
                    : false
                }
                data={filteredData}
