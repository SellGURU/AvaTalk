...
const validationYup = (property:string) => {
    switch(property) {
        case 'company':
            return Yup.string().min(3,'Company name must be between 3 and 15 characters.').max(15,'Company name must be between 3 and 15 characters.').trim('Company Name must not be empty or just spaces.')
        case 'job':
            return Yup.string().min(3,'Job title must be between 3 and 15 characters.').max(15,'Job title must be between 3 and 15 characters.').trim('Job must not be empty or just spaces.')
        case 'fullName':
